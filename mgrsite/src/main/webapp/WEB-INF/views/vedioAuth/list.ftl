...
<head>
	...
	$(function(){
				...
				$.ajax({
					...
					success:function(data){
						$.each(data,function(index,item){
							$('<li data-uid="'+item.id+'"><a class="autocomplate_item" href="javascript:;">'+item.name+'</a></li>').appendTo($(".autocomplate"));
						});
		...
		$("#autocomplate").on("show.bs.dropdown",function(){
			if($("[name=loginInfoDisplay]").val()<3)return false;
		});
	});
	</script>
</head>
			...
			<div class="col-sm-9">
				...
				<div class="row">
					...
					<form id="searchForm" class="form-inline" method="post" action="/vedioAuth">
						...
						<div class="form-group">
						    <label>申请时间</label>
						    <input class="form-control" style="width: 130px;" type="text" name="beginDate" id="beginDate" value="${(qo.beginTime?string('yyyy-MM-dd'))!''}" />到
						    <input class="form-control" style="width: 130px;" type="text" name="endDate" id="endDate" value="${(qo.endTime?string('yyyy-MM-dd'))!''}" />
						</div>
						<div class="form-group">
